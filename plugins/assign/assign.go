...
func helpProvider(config *plugins.Configuration, enabledRepos []string) (*pluginhelp.PluginHelp, error) {
	...
	fmt.Println("help provider", config)
	fmt.Println("help provider", enabledRepos)
...
func handleGenericComment(pc plugins.Agent, e github.GenericCommentEvent) error {
	...
	err := handle(newAssignHandler(e, *pc.GitHubClient, pc.Logger, github.Client{}))
	if e.IsPR {
		err = combineErrors(err, prhandle(newReviewHandler(e, *pc.GitHubClient, pc.Logger)))
	}
	return err
}
...
type handler struct {
	...
	remove func(ctx context.Context, org, repo string, number int, users []string) (*hub.Issue, *hub.Response, error)
	// add is the function that is called on the affected logins for a command with no 'un' prefix.
	add func(ctx context.Context, org, repo string, number int, users []string) (*hub.Issue, *hub.Response, error)

	pradd func(ctx context.Context, org, repo string, number int, users []string) ( *hub.PullRequest, *hub.Response, error)

	prremove func(ctx context.Context, org, repo string, number int, users []string) (*hub.Response, error)
...
func newAssignHandler(e github.GenericCommentEvent, client hub.Client, log *logrus.Entry, gc github.Client) *handler {
	...
	return &handler{
		...
		remove:             client.Issues.RemoveAssignees,
		add:                client.Issues.AddAssignees,
		event:              &e,
		regexp:             assignRe,
		gc:                 gc,
		client:             client,
...
func handle(h *handler) error {
	...
	if len(toRemove) > 0 {
		...
		if _, _, err := h.remove(ctx, org, repo, e.Number, toRemove); err != nil {
			return err
		}
	}
	if len(toAdd) > 0 {
		h.log.Printf("Adding %s to %s/%s#%d: %v", h.userType, org, repo, e.Number, toAdd)
		if _, _, err := h.add(ctx, org, repo, e.Number, toAdd); err != nil {
...
func prhandle(h *prhandler) error {
	...
	var toPRAdd, toPRRemove []string
	for login, pradd := range users {
		if pradd {
			toPRAdd = append(toPRAdd, login)
		} else {
			toPRRemove = append(toPRRemove, login)
		}
	}

	if len(toPRRemove) > 0 {
		h.log.Printf("Removing %s from %s/%s#%d: %v", h.userType, org, repo, e.Number, toPRRemove)
		if _,err := h.prremove(ctx, org, repo, e.Number, toPRRemove); err != nil {
			return err
		}
	...
	if len(toPRAdd) > 0 {
		h.log.Printf("Adding %s to %s/%s#%d: %v", h.userType, org, repo, e.Number, toPRAdd)
		if _,_,err := h.pradd(ctx, org, repo, e.Number, toPRAdd); err != nil {
			if mu, ok := err.(github.MissingUsers); ok {
				msg := h.addFailureResponse(mu)
				if len(msg) == 0 {
					return nil
...
func newReviewHandler(e github.GenericCommentEvent, client hub.Client, log *logrus.Entry) *prhandler {
	org := e.Repo.Owner.Login
	addFailureResponse := func(mu github.MissingUsers) string {
	...
	return &prhandler{
		addFailureResponse: addFailureResponse,
		prremove:	    client.PullRequests.RemoveReviewers,
		pradd:		    client.PullRequests.RequestReviewers,
		event:  &e,
		regexp: CCRegexp,
		//gc:                 gc,
		log:      log,
