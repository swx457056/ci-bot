...
func NewAgent(configAgent *config.Agent, pluginConfigAgent *ConfigAgent, clientAgent *hub.Client, logger *logrus.Entry) Agent {
	prowConfig := configAgent.Config()
	pluginConfig := pluginConfigAgent.Config()
	return Agent{
		GitHubClient: clientAgent,
		Config:       prowConfig,
...
func (pa *ConfigAgent) Load(path string) error {
	...
	pa.Set(np)
	//	fmt.Println("after Set method", pa.configuration)
	return nil
}
...
func (pa *ConfigAgent) getPlugins(owner, repo string) []string {
	...
	plugins = append(plugins, pa.configuration.Plugins[repo]...)
